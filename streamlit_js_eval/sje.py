...
streamlit_js_eval = components.declare_component(
    "streamlit_js_eval", path=frontend_path
)
...
def get_user_agent(component_key=None):
    ...
    return streamlit_js_eval(
        js_expressions=f"window.navigator.userAgent", key=component_key
    )
...
def copy_to_clipboard(copiedText, linkText, successText, component_key=None):
    ...
    if component_key is None:
        component_key = f"{linkText}{copiedText}{successText}"
...
def get_browser_language(component_key=None):
    if component_key is None:
        component_key = "LANG"
    return streamlit_js_eval(
        js_expressions=f"window.navigator.language", key=component_key
    )
...
def get_page_location(component_key=None):
    ...
    return json.loads(
        streamlit_js_eval(
            js_expressions="JSON.stringify(window.location)", key=component_key
        )
    )
...
def create_share_link(sharedObject, linkText, successText, component_key=None):
    ...
    if component_key is None:
        component_key = f"{linkText}{sharedObject}{successText}"
